...
void TCPConnection::pack_outbound_segment(TCPSegment &outbound_seg,
                                          bool send_rst) {
...
void TCPConnection::segment_received(const TCPSegment &seg) {
    ...
    if (res_receiver) {
        ...
        // Prereq #3: The outbound stream has been fully acknowledged by the
        // remote peer.
...
void TCPConnection::send_to_outbound(bool send_syn, bool send_rst) {
    ...
    while (!_sender.segments_out().empty()) {
        ...
        // cout << ">>> seg sent: " << seg_send.header().summary() << endl;
...
void TCPConnection::tick(const size_t ms_since_last_tick) {
    ...
    if (_end_receiver && _end_sender &&
        _time_cur - _last_recieved >= 10 * _cfg.rt_timeout) {
...
void TCPConnection::connect() { send_to_outbound(true); }
