...
void TCPSender::fill_window() {
    ...
    while (!_fin_sent && _window_end > _next_seqno) {
        TCPSegment segment{};
        size_t data_length =
            min(TCPConfig::MAX_PAYLOAD_SIZE, _window_end - _next_seqno);
        Buffer buffer(_stream.read(data_length));
        segment.payload() = buffer;
        if (_window_end - _next_seqno > segment.length_in_sequence_space() &&
            _stream.eof()) {
            segment.header().fin = true;
            _fin_sent = true;
        }
        // cerr << "here the buffer: " << buffer.str() << " | here the fin: " <<
        // segment.header().fin << endl;
...
bool TCPSender::ack_received(const WrappingInt32 ackno,
...
void TCPSender::tick(const size_t ms_since_last_tick) {
    ...
    if (_timer_set &&
        _time_miliseconds - _timer_start >= _retransmission_timeout) {
        //! retransmission
        TCPSegment segment{};
...
bool RetransmissionQueue::empty() { return _segments_queue.empty(); }
